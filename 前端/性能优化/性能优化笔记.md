...
![](PublicImage/Pasted%20image%2020240721164154.png)
...
![](PublicImage/Pasted%20image%2020240723002936.png)
其兼容性（偏差，所以大部分是gzip
...
```
![](PublicImage/Pasted%20image%2020240723004905.png)
这是路径
...
## 数据收集
一般情况下采用web-vitals+grafana进行数据的收集与展示。
但是如果需要手写的话，可以自行实现


#### FCP
**定义**：首次内容绘制时间，即页面开始加载到首次渲染出文本、图片、非空白 Canvas/SVG 的时间。理想值应 ≤1.8 秒
​**关键内容**：文本、图片、非白色 `<canvas>` 或 `<svg>` 元素
```javascript
...
#### LCP
**定义**：衡量视口内最大可见元素（如文本块、图片、视频等）完成渲染的时间，反映用户感知的主要内容加载速度。理想值应 ≤2.5 秒
- **关键元素**：包括 `<img>`、`<video>`、背景图、包含文本的块级元素等
- ​**排除条件**：不可见元素（如 `opacity: 0`）、全视口覆盖元素、低熵图片等
```javascript
let lcpValue = 0;
const observer = new PerformanceObserver((entryList) => {
  ...
  entries.forEach(entry => {
    if (entry.startTime > lcpValue) {
      lcpValue = entry.startTime; // 取最大的渲染时间 这里取startTime是因为观察的类型为largest-contentful-paint
...
#### TTI
**定义**：页面从加载到完全可交互的时间，需满足以下条件：
1. 首次内容绘制（FCP）已完成。
2. 主线程连续 5 秒无长任务（执行时间超过 50ms 的任务）。
3. 同时未超过 2 个进行中的网络请求
...
const longTaskObserver = new PerformanceObserver((entryList) => {
  const lastLongTask = entryList.getEntries().pop();
  const taskEndTime = lastLongTask.startTime + lastLongTask.duration;
  
  // 3. 检查是否满足连续 5 秒无长任务
  if (taskEndTime >= fcpTime && taskEndTime + 5000 <= performance.now()) {
    tti = taskEndTime;
    console.log('TTI:', tti);
...
#### INP
**定义**：衡量用户交互（如点击、输入）到页面视觉反馈（下一帧渲染）的延迟时间，反映页面响应速度。理想值应 ≤200ms
```js
let interactionStartTime = 0;
let inpValue = 0;

// 监听用户交互事件（点击、输入、键盘事件等）
['click', 'keydown', 'input'].forEach(type => {
  document.addEventListener(type, (event) => {
    interactionStartTime = performance.now();
  }, { capture: true, passive: true }); //捕获阶段（capture: true）和被动模式（passive: true）
});

// 通过 PerformanceObserver 监听下一帧渲染
const observer = new PerformanceObserver((entries) => {
  entries.getEntries().forEach(entry => {
    if (interactionStartTime > 0) {
      const delay = entry.startTime - interactionStartTime;
      if (delay > inpValue) inpValue = delay; // 记录最慢的交互延迟
...
#### CLS
**定义**：累积布局偏移分数，衡量页面元素意外移动对用户体验的影响。理想值应 ≤0.1
```
CLS = 偏移分数（impact fraction） × 距离分数（distance fraction）
```

- ​**偏移分数**：受影响元素在视口中的面积占比。
- ​**距离分数**：元素移动的最大距离占视口宽/高的比例（取最大值）。
```js
let clsValue = 0;
const observer = new PerformanceObserver((entries) => {
  entries.getEntries().forEach(entry => {
...
#### TTFB
**定义**：从发起请求到接收到服务器第一个字节的时间，反映服务器响应速度。理想值应 ≤600ms。
```javascript
...
```
#### API的差异
PerformanceObserver 和 Performance API 是浏览器提供的用于监控和分析网页性能的核心工具，能够帮助开发者精确测量关键性能指标（如 FCP、LCP、CLS 等）。以下是它们的核心功能和使用场景介绍：

---

##### 一、**Performance API 的核心功能**

Performance API 提供了对网页性能数据的底层访问能力，包含以下核心接口：

###### 1. ​**Performance 对象**

- ​**功能**：全局对象 `performance` 是性能数据的入口，提供多种方法获取性能条目（PerformanceEntry），例如：
    - `performance.getEntries()`：获取所有性能条目（如资源加载、页面导航等）
    - `performance.mark()` / `performance.measure()`：手动标记代码执行时间点或测量时间段
- ​**指标类型**：
    - ​**页面导航指标**​（`navigation`）：记录页面加载过程中的关键时间节点（如 DNS 查询、DOM 解析、TTFB 等）
    - ​**资源加载指标**​（`resource`）：监控 CSS、JS、图片等资源的加载耗时和大小
    - ​**渲染指标**​（`paint`）：包括首次绘制（FP）和首次内容绘制（FCP）时间
###### 2. ​**PerformanceEntry 对象**
- ​**描述**：封装单个性能指标数据，所有性能条目（如 `navigation`、`resource`）均继承自此类
- ​**关键属性**：
    - `entryType`：条目类型（如 `paint`、`navigation`）。
    - `name`：资源 URL 或自定义标记名称。
    - `startTime` / `duration`：时间戳和执行时长

---

##### 二、**PerformanceObserver 的作用与优势**

PerformanceObserver 是用于**异步监听性能事件**的 API，解决了传统 `performance.getEntries()` 需轮询获取数据的低效问题
###### 1. ​**核心功能**

- ​**事件驱动**：当监听的性能条目（如 `paint`、`longtask`）被记录时，自动触发回调函数

- ​**支持多种条目类型**：包括关键性能指标（如 `largest-contentful-paint`、`layout-shift`）和自定义事件

- ​**方法**：
    - `observe()`：指定监听的条目类型（如 `entryTypes: ['paint']`）
    - `disconnect()`：停止监听。
    - `takeRecords()`：获取当前缓存条目
