...
Lox 既有 expressions 也有 statements。某些语言省略了后者。相反，它们也将声明和控制流构造视为表达式。这些 “一切都是表达式” 的语言往往具有功能血统，包括大多数 Lisps、SML、Haskell、Ruby 和 CoffeeScript。

取消语句的语言通常还具有**隐式返回**功能，即函数自动返回其主体计算的任何值，而无需一些显式`的返回`语法。对于小函数和方法，这真的很方便。事实上，许多具有 statements 的语言都添加了语法，例如 `=>`，以便能够定义其主体是计算单个表达式的结果的函数。



# 解释器
## 扫描 Scanning
作用：扫描程序将原始源代码作为一系列字符接收，并将其分组为一系列我们称为**标记**的块。
